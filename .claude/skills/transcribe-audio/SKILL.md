...
# Transcribe Audio (hark, local-only)

Transcribe audio with **hark**, a local faster-whisper CLI. Audio never
leaves the machine — do not send audio to cloud transcription APIs, and do
not write inline whisper one-liners (they reload the model per file; hark
loads it once per batch).

Invocation: use `hark ...` if the command is on PATH (installed via
`uv tool install .`); otherwise run `uv run hark ...` from the repo root.

```bash
# Batch: files, dirs, or globs (model loads once per run)
hark ~/Downloads/audios/*.opus --out /path/to/transcripts

# WhatsApp export zip: no manual unzip needed; dates inclusive
# (quote paths with spaces using $HOME — a quoted ~ does not expand)
hark whatsapp "$HOME/Downloads/WhatsApp Chat - X.zip" \
