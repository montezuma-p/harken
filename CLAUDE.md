...
- **`pr-run-mode = "upload"`**: a pull request builds the full 5-target matrix
  without publishing. Use a PR to rehearse anything that touches the release
  path — the toolchain pin, `build.rs`, the submodule — instead of finding out
  from a red `v*` tag, which is how v0.4.0 died.
